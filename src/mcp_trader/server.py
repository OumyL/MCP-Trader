...
"""
    ...
    async def compare_companies(
        ...
        try:
            ...
            # Get comparison data
            comparison = await fundamental_analyzer.compare_companies(symbol_list, metrics_list)
            
            if "error" in comparison:
                return f"❌ Error in comparison: {comparison['error']}"
            
            ctx.report_progress(0.8, "Generating comparison report...")
            
            comparison_text = f"""
📊 **Company Comparison** ({comparison["companies_analyzed"]} companies)

**📈 Ranking by Valuation Score:**
"""
            
            for company in comparison["comparison_table"]:
...
"""
            ...
            analysis_date = comparison["analysis_date"][:10] if "analysis_date" in comparison else "N/A"
...
"""
            ...
            await log_request_end(ctx, request_id, "compare_companies", True, f"Compared {len(symbol_list)} companies")
...
async def compare_companies(
    ...
    try:
        ...
        # Auto-limiter pour éviter les quotas Alpha Vantage (5 calls/minute)
        original_count = len(symbol_list)
        if len(symbol_list) > 3:
            symbol_list = symbol_list[:3]
            ctx.report_progress(0.1, f"Limited to 3 companies (Alpha Vantage: 5 calls/minute limit)...")
        else:
            ctx.report_progress(0.1, f"Comparing {len(symbol_list)} companies...")
        ...
        for i, symbol in enumerate(symbol_list):
            progress_start = 0.1 + (0.7 * i / len(symbol_list))
            progress_end = 0.1 + (0.7 * (i + 1) / len(symbol_list))
            
            ctx.report_progress(progress_start, f"Analyzing {symbol}... ({i+1}/{len(symbol_list)})")
            
            # Délai intelligent Alpha Vantage 
            if i > 0:
                delay = 13 + (i * 2)  
                ctx.report_progress(progress_start + 0.02, f"Waiting {delay}s for API limits...")
                await asyncio.sleep(delay)
            
            max_retries = 4 
            for attempt in range(max_retries):
                try:
                    timeout_duration = 25 + (attempt * 15)  # 25s, 40s, 55s, 70s                    ctx.report_progress(progress_start + 0.05, f"Fetching {symbol} data (attempt {attempt+1})...")
                    
                    # Délai adaptatif selon les tentatives
                    if attempt > 0:
                        delay = 10 + (attempt * 10)  # 10s, 20s, 30s
                        ctx.report_progress(progress_start + 0.02, f"Waiting {delay}s before retry {attempt+1}...")
                        await asyncio.sleep(delay)
        
                    ctx.report_progress(progress_start + 0.05, f"Fetching {symbol} data (attempt {attempt+1}/{max_retries})...")
        
                    # Analysis avec timeout plus long
                    analysis = await asyncio.wait_for(
                        fundamental_analyzer.get_comprehensive_analysis(symbol),
                        timeout=timeout_duration
                    )
                    if "error" not in analysis:
                        companies_data[symbol] = analysis
                        success_count += 1
                        ctx.report_progress(progress_end - 0.05, f"✅ {symbol} completed successfully")
                        logger.info(f"Successfully analyzed {symbol}")
                        break  # Sortir du retry loop
                    else:
                        logger.warning(f"Analysis error for {symbol} (attempt {attempt+1}): {analysis['error']}")
                        if attempt == max_retries - 1:
                            failed_companies.append((symbol, analysis['error'][:50]))
                            
                except asyncio.TimeoutError:
                    timeout_msg = f"Timeout after {timeout_duration}s"
                    logger.warning(f"Timeout for {symbol} (attempt {attempt+1})")
                    ctx.report_progress(progress_start + 0.08, f"⏰ {symbol} timeout - retrying...")
                    if attempt == max_retries - 1:
                        failed_companies.append((symbol, timeout_msg))
                        
                except Exception as e:
                    error_str = str(e)
                    logger.error(f"Error analyzing {symbol} (attempt {attempt+1}): {error_str}")
                    
                    # Gestion spéciale des erreurs Alpha Vantage
                    if any(term in error_str.lower() for term in ["limit", "429", "quota", "rate"]):
                        if attempt == max_retries - 1:
                            failed_companies.append((symbol, "API rate limit exceeded"))
                        else:
                            # Attendre plus longtemps pour les limites API
                            await asyncio.sleep(20)
                    else:
                        if attempt == max_retries - 1:
                            failed_companies.append((symbol, error_str[:50]))
        
        ctx.report_progress(0.8, "Creating comparison report...")
        
        # Gestion intelligente des résultats
        if not companies_data:
            failed_details = "\n".join([f"• **{sym}:** {err}" for sym, err in failed_companies])
            return f"""❌ **Company Comparison - Complete Failure**

**Requested:** {', '.join(symbol_list)} ({original_count} total)
**Result:** 0/{len(symbol_list)} companies analyzed successfully

**Failure Analysis:**
{failed_details}

**🔧 Alpha Vantage Limits (Free Tier):**
• **5 calls per minute** (need 12+ seconds between calls)
• **500 calls per day** total
• **Premium tier** removes these limits

**💡 Smart Solutions:**
1. **Try 1 company:** `compare_companies("AAPL")`
2. **Wait 5+ minutes** then retry
3. **Sequential approach:** Use `analyze_fundamentals("AAPL")` individually
4. **Upgrade to Alpha Vantage Premium** for bulk analysis

**📊 Alternative:** Use technical analysis tools which don't have these limits
"""
        
        # Créer le tableau de comparaison pour les réussites
        comparison_table = []
        ...
        for symbol, data in companies_data.items():
            try:
                ...
                company_row = {
                    ...
                    "recommendation": valuation["recommendation"],
                    # Métriques avec gestion des None
                    "pe_ratio": getattr(overview, 'pe_ratio', None),
                    "pb_ratio": getattr(overview, 'pb_ratio', None),
                    "roe": getattr(overview, 'roe', None),
                    "profit_margin": getattr(overview, 'profit_margin', None),
                    "debt_to_equity": getattr(overview, 'debt_to_equity', None)
                }
                ...
                failed_companies.append((symbol, f"Data processing error"))
        ...
        success_rate = success_count / original_count * 100
...
🔢 **Analysis Summary:**
• **Requested:** {original_count} companies
• **Processed:** {len(symbol_list)} (API limit protection)
• **Successful:** {success_count} companies ✅
• **Failed:** {len(failed_companies)} companies ❌
• **Success Rate:** {success_rate:.0f}%

"""
        
        # Ajouter les résultats s'il y en a
        if comparison_table:
            comparison_text += "**📈 Fundamental Analysis Ranking:**\n"
            
            for company in comparison_table:
                recommendation_emoji = {
                    "ACHAT FORT": "🚀", "ACHAT": "📈", "CONSERVER": "⚖️",
                    "VENDRE": "📉", "VENTE FORTE": "🔻"
                }.get(company["recommendation"], "⚖️")
                
                # Format market cap safely
                if company['market_cap']:
                    market_cap_text = f"${company['market_cap']/1e9:.1f}B"
                else:
                    market_cap_text = "N/A"
...
"""
                ...
                for metric in metrics_list:
                    if metric in company and company[metric] is not None:
                        ...
                        comparison_text += f"• **{metric.upper()}:** N/A\n"
                
                comparison_text += "\n"
        
        # Ajouter les échecs avec conseils
        if failed_companies:
            comparison_text += f"⚠️ **Analysis Failures ({len(failed_companies)}):**\n"
            for symbol, error in failed_companies:
                comparison_text += f"• **{symbol}:** {error}\n"
            
            comparison_text += "\n**🔧 Solutions for Failed Companies:**\n"
            comparison_text += "• **Individual analysis:** Use `analyze_fundamentals(\"SYMBOL\")` one by one\n"
            comparison_text += "• **Wait longer:** 15+ minutes between bulk requests\n"
            comparison_text += "• **Smaller batches:** Max 2 companies per request\n\n"
        
        # Informations finales
        comparison_text += f"""**📊 Analysis Details:**
• **Date:** {datetime.now().strftime('%Y-%m-%d %H:%M')}
• **Metrics:** {", ".join(metrics_list)}
• **Data Source:** Alpha Vantage API
• **Processing Time:** ~{len(symbol_list) * 15} seconds (rate limiting)

**💡 Pro Tips:**
• **Best Success Rate:** 1-2 companies per request
• **Bulk Analysis:** Consider Alpha Vantage Premium
• **Alternative:** Technical analysis has no rate limits

**🎯 Overall Assessment:** {success_count}/{original_count} companies analyzed - {'Excellent' if success_rate >= 80 else 'Good' if success_rate >= 50 else 'Limited'} success rate
"""
        
        ctx.report_progress(1.0, "Comparison complete!")
        success_msg = f"Analyzed {success_count}/{original_count} companies successfully"
        await log_request_end(ctx, request_id, "compare_companies", True, success_msg)
        return comparison_text
        
    except Exception as e:
        error_msg = await handle_error(ctx, request_id, "compare_companies", e)
        return f"❌ Critical error in company comparison: {error_msg}\n\n💡 **Try:** Individual analysis with `analyze_fundamentals(\"SYMBOL\")`"
...
async def get_final_recommendation(
    ...
    try:
        ...
        if fundamental_analyzer and analysis_type in ["complete", "fundamental"]:
            ctx.report_progress(0.5, "Running fundamental analysis...")
            try:
                analysis = await fundamental_analyzer.get_comprehensive_analysis(symbol)
...
"""
        ...
        ctx.report_progress(0.5, "Processing technical indicators...")
